pub fn solution(input: String) -> i64 {
  ...
  let seed_ranges = match parse_seed_ranges(&blocks[0]) {
      Some(ranges) => ranges,
      None => {
          return -1;
      }
  ...
  for (start, length) in seed_ranges {
      all_seeds.extend(start..start + length);
  }

  let mut maps: Vec<Vec<((i64, i64), i64)>> = Vec::new();
  for block in &blocks[1..=7] {
      let map = parse_map_to_ranges(block);
      maps.push(map);
  }

  let smallest = all_seeds
      .iter()
...
fn parse_seed_ranges(input: &str) -> Option<Vec<(i64, i64)>> {
  input
      .split(": ")
      .nth(1)?
      .split_whitespace()
